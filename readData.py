...
def main():
    ...
    for index,row in df.iterrows():
        ...
        pp.pprint(dfTitle.head())
        pp.pprint(dfTitle.index)
        # pp.pprint(dfDesc)
        ...
        pp.pprint("Retweeted Url: %s" % (row["retweeted_url"]))
        pp.pprint("Means:")
        ...
        # Data must have at least 10% os variety to compare
        if ( (df.shape[0] / dfTitle.shape[0]) >= 10 and (df.shape[0] / dfDesc.shape[0]) >= 10 ):
            pp.pprint("Data variety insufficient for collection: %s" % (row['collection_name']))
            continue
